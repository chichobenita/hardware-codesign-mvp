...
describe('review/handoff UI flow', () => {
  ...
  it('renders previously created handoff history with correct module/provider context', () => {
    ...
    state.handoffArtifacts = [
      {
        artifactId: 'handoff_example_uart_rx_older',
        schemaVersion: HANDOFF_ARTIFACT_SCHEMA_VERSION,
        moduleId: 'example_uart_rx',
        moduleName: 'uart_rx',
        createdAt: '2026-03-18T11:00:00.000Z',
        targetProviderId: MOCK_PROVIDER_ID,
        handoffStatus: 'handed_off',
        consistencyMarker: createArtifactConsistencyMarkerFromState(state, 'example_uart_rx') ?? 'hf_test_older',
        ...
        promptSnapshot: {
          title: 'Older HDL prompt for uart_rx',
  ...
  it('shows the newest handoff artifact first even if persisted history is out of order', () => {
    const state = createEligibleHandoffState();
    state.handoffArtifacts = [
      {
        artifactId: 'handoff_example_uart_rx_older',
      ...
      {
        artifactId: 'handoff_example_uart_rx_newer',
        schemaVersion: HANDOFF_ARTIFACT_SCHEMA_VERSION,
        moduleId: 'example_uart_rx',
        moduleName: 'uart_rx',
        createdAt: '2026-03-18T12:00:00.000Z',
        targetProviderId: MOCK_STRUCTURED_PROVIDER_ID,
        handoffStatus: 'handed_off',
        consistencyMarker: createArtifactConsistencyMarkerFromState(state, 'example_uart_rx') ?? 'hf_test_newer',
        generationPayloadSnapshot: {
          module_name: 'uart_rx',
          ports: [],
          purpose: '',
        ...
        promptSnapshot: {
          title: 'Newer HDL prompt for uart_rx',
    ...
    expect(screen.getByText(/"artifactId": "handoff_example_uart_rx_newer"/)).toBeInTheDocument();
  });
